const GyeonggiArray =[
  {"order":1,"name":"고양시","en":"Goyang-si"},
  {"order":2,"name":"과천시","en":"Gwacheon-si"},
  {"order":3,"name":"광명시","en":"Gwangmyeong-si"},
  {"order":4,"name":"광주시","en":"Gwangju-si"},
  {"order":5,"name":"구리시","en":"Guri-si"},
  {"order":6,"name":"군포시","en":"Gunpo-si"},
  {"order":7,"name":"김포시","en":"Gimpo-si"},
  {"order":8,"name":"남양주시","en":"Namyangju-si"},
  {"order":9,"name":"동두천시","en":"Dongducheon-si"},
  {"order":10,"name":"부천시","en":"Bucheon-si"},
  {"order":11,"name":"성남시","en":"Seongnam-si"},
  {"order":12,"name":"수원시","en":"Suwon-si"},
  {"order":13,"name":"시흥시","en":"Siheung-si"},
  {"order":14,"name":"안산시","en":"Ansan-si"},
  {"order":15,"name":"안성시","en":"Anseong-si"},
  {"order":16,"name":"안양시","en":"Anyang-si"},
  {"order":17,"name":"양주시","en":"Yangju-si"},
  {"order":18,"name":"여주시","en":"Yeoju-si"},
  {"order":19,"name":"오산시","en":"Osan-si"},
  {"order":20,"name":"용인시","en":"Yongin-si"},
  {"order":21,"name":"의왕시","en":"Uiwang-si"},
  {"order":22,"name":"의정부시","en":"Uijeongbu-si"},
  {"order":23,"name":"이천시","en":"Icheon-si"},
  {"order":24,"name":"파주시","en":"Paju-si"},
  {"order":25,"name":"평택시","en":"Pyeongtaek-si"},
  {"order":26,"name":"포천시","en":"Pocheon-si"},
  {"order":27,"name":"하남시","en":"Hanam-si"},
  {"order":28,"name":"화성시","en":"Hwaseong-si"},
  {"order":29,"name":"가평군","en":"Gapyeong-gun"},
  {"order":30,"name":"양평군","en":"Yangpyeong-gun"},
  {"order":31,"name":"연천군","en":"Yeoncheon-gun"}
]
const bigCities = [
  {name: "Gangwon-do"},
  {name: "Gyeonggi-do"},
  ...
  return new Promise((resolve, reject) => {
    const res = $.ajax({
      ...
      success: async function(data){
        const air = await apiCall_air()
        const result = {"city":city, "temp": data.main.temp, "humid": data.main.humidity, "wind":data.wind.speed, "cloud": data.clouds.all,  "snow":data.snow, "air":air, "location":{"x":data.coord.lon, "y":data.coord.lat}};
        if(data.rain){
...
const drawRows = async (areaArray) => {
  ...
  result.forEach((a) => {
    const myArr = [a.temp, Math.round(a.humid)*0.01, a.wind, Math.round(a.cloud)*0.01, a.snow, a.rain, 0];
  ...
  for(let i = 0; i < 10; i++){
    const pointX = result[i].location.x;
    const pointY = result[i].location.y;
    const distanceScore = await kimin(pointX, pointY);
    if(!result[i].rain){
      result[i].rain = "-";
    } else result[i].rain = result[i].rain+"mm";
    if(!result[i].snow){
      result[i].snow = "-";
    } else result[i].snow = result[i].snow+"mm";
    $(`<div class='divTR'>
        <div class='divTD'>${order}</div>
        <div class='divTD'>${result[i].city}</div>
        <div class='divTD'><b>${result[i].weatherScore}</b></div>
        <div class='divTD'><b>${distanceScore}</b></div>
        <div class='divTD'>${result[i].temp}℃</div>
        <div class='divTD'>${Math.round(result[i].cloud)}%</div>
        <div class='divTD'>${result[i].wind}m/s</div>
        <div class='divTD'>${Math.round(result[i].humid)}%</div>
        <div class='divTD'>${result[i].rain}</div>
        <div class='divTD'>${result[i].snow}</div>
        <div class='divTD'>${result[i].air}</div>
...
const weatherArr = [20, 0.6, 4, 0, 0, 0, 0];
