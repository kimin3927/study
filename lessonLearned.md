...
- scroll : 스크롤 표시
- auto : over-flow일때만 scroll 표시
- hidden : over-flow부분은 아예 안보여줌
- visible : 스크롤 바는 없지만 스크롤 기능이 작동하는 상태 



* 특이사항 : scroll 혹은 auto로 인해 scrollBar가 구현되는 경우, 해당 부분역시 width와 height를 차지하게 됨.

  이로 인해 html : 100vw, 100vh로 설정했는데도 사이즈보다 크게 되는 일이 일어날 수 있음 
...
Jquery야 .css로 간단히 style이 확인됐는데

자바스크립트로 하려니, 

style.propertyName... 이 안먹히더라. 

이유는, style.propertyName는  inline style에 관한 것이었고, 내가 알고 있는 style들은 styleSheet에 있었기 때문이다.
...
flat()은 중첩 배열 구조를 평탄화할 때 쓰는 메서드이다. 
...
```
const canvas = document.getElementById('canvas'); // 그릴 위치를 가질 컨테이너를 참조
const ctx = canvas.getContext('2d'); // context(2d or 3d)부여 및 그려질 해당 객체 생성
이후 ctx에 각종 속성 부여하여 그림 구체화
```
...
- 관련 api 일부(튜토리얼)
  ...
  - ctx.fillRect(10, 10, 150, 100); // 시작표인트(x, y) + 사이즈(width, height) 로 구성 의미
  
  - ctx.fillRect // 속이 꽉찬 사각형
  
  - ctx.clearRect // 속이 텅빈 사각형
  
  - ctx.strokeRect // 윤곽선만 있는 사각형
  
  - ctx.beginPath(); // 도형 그리기 따위를 위해 path를 구성할 경우 start한다는 함수 (좌표 불필요)
  
  - ctx.moveTo(); // beginPath() 실행 이후 start 좌표 지정
  
  - ctx.lineTo(좌표) // 스타트 포인트에서 특정좌표로 line그리기
  
  - ctx.fill();  다 그린이후 채우기

- 그대로 api를 활용하기보다 라이브러리를 사용하는것이 보편적. 

## 82. JS) 라이브러리 차트그리기

> 차트를 그리려고 할 경우 canvas태그 등을 활용할 수 있지만 라이브러리를 사용하는것이 보편적 

다양한 관련 라이브러리

1. ***uPlot.js** 『https://github.com/leeoniya/uPlot』
2. ***chart.js** 『https://www.chartjs.org/』
3. ***Amcharts** 『https://www.amcharts.com/』
4. ***Zingchart**『https://www.zingchart.com/』
5. ***Apexcharts.js 『**https://apexcharts.com/』

chart.js 시도해보기 

1. 해당라이브러리 다운로드(or CDN)
2. canvas 태그 생성
3. new Chart 객체를 통해 컨트롤 
4. 기타 속성 name 
   1. type : 그래프 종류
   2. data : data 
      1. labels : x 값
      2. datasets : 데이터 구성값
         1. label
         2. lineTension  : 기본값 true // false(0) 설정 시 직선 그래프
         3. data
         4. backgroundColor
         5. pointStyle : 점 스타일
         6. pointBackgroundColor : 점 채우기 색상
         7. showLine :  데이터 집합에 대해 선이 여부
         8. spanGaps : 데이터가  null 일 경우에  사이에 선이 그려질지 여부
         9. steppedLine : 계단식으로 표현할 지 여부

## 83. 조건문을 위한 true false 확인

- if(!condition)이라는 조건문을 쓰려 할경우에
  1. if(!null) == true?
  2. if(!undefine) == true?
  3. if("") == false? 
